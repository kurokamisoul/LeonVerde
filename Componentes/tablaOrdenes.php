    ...
    <font color=white><strong> 
		...
		<table class="table table-hover table-condensed table-bordered" id="tablaDinamicaLoad" >
			...
			<tbody style="background-color:#FFFF!important; color:#000000">
				...
				$sentencia=$pdo->prepare("SELECT id_orden,cliente,correo,telefono,total,fechaOrden FROM ordenes");
					
                    
                    $sentencia->execute();
                    $listaOrdenes=$sentencia->fetchAll(PDO::FETCH_ASSOC);
			...
			<button class="btn btn-warning glyphicon glyphicon-list" data-toggle="modal" data-target="#modalDetalle" onclick="verDetalles(<?php echo $orden ['id_orden'];?>)" ></button>
...
<script>
function detalle2(modal){
	var options = {
			modal: true,
			height:300,
			width:600
		};
	$('#modalDetalle').load('tablaLibrosOrdenes.php?dato='+modal, function() {
